...
Vartovyi — фоновий моніторинг Telegram-сповіщень з повноекранною тривогою при збігу з вашими
ключовими словами. Створений для нічного чергування під час повітряних тривог в Україні.

> `google-services.json` не включено в репозиторій — для локального збирання потрібен власний
> Firebase проєкт.
...
Плагіни підключаються через `alias(libs.plugins.…)`.
...
і тексту (виправлення опечаток). Сигнатура — SHA-256 від
`pkg | notificationKey | senderName | messageText` (нормалізований текст).
Дедуп працює як **ковзне 60-секундне вікно**:
...
- нормалізований текст повідомлення (lowercase, trim, collapse whitespace,
  strip invisible unicode) стабільний між Telegram refresh-edit циклами — різні
  реальні повідомлення мають різні сигнатури і не зливаються;
...
- пріоритет — **не втратити жодне реальне повідомлення**, тому ризик мінімальний:
  втрата можлива лише якщо два різних повідомлення мають однаковий нормалізований
  текст від того самого відправника в межах 60с (теоретичний edge case).
...
- сигнатура з точним `notification.when` (Telegram зсуває `when` → дубль на
  кожен refresh-edit);
- `messages.size` як частина сигнатури (старі записи з тим самим size після
  reset Telegram колізіонували з новими і губилися).
